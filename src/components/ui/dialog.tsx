...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 text-foreground/30 hover:text-foreground/60 transition-colors focus:outline-none disabled:pointer-events-none">
        <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn("text-sm font-light tracking-[0.15em] uppercase text-foreground", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description
    ref={ref}
    className={cn("text-xs text-foreground/40 font-light", className)}
    {...props}
  />
));
